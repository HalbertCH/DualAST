...
def save_batch(input_painting_batch, input_photo_batch, output_photo_batch, filepath):
    """
    ...
    Args:
        ...
        output_photo_batch: numpy array of size [(2*B) x H x W x C]
    ...
    inputs = np.concatenate([batch_to_img(input_painting_batch), batch_to_img(input_photo_batch)],
                            axis=0)
    outputs = np.concatenate([batch_to_img(input_painting_batch), batch_to_img(output_photo_batch)],
                             axis=0)
