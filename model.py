...
class Artgan(object):
    def __init__(self, sess, args):
        ...
        OPTIONS = namedtuple('OPTIONS',
                             'batch_size image_size \
                              ...
                              path_to_art_dataset \
                              discr_loss_weight transformer_loss_weight style_loss_weight content_feature_loss_weight \
                              content_feature_adv_loss_weight tv_loss_weight')
        self.options = OPTIONS._make((args.batch_size, args.image_size,
                                      ...
                                      args.discr_loss_weight, args.transformer_loss_weight, args.style_loss_weight, args.content_feature_loss_weight,
                                      args.content_feature_adv_loss_weight, args.tv_loss_weight
                                      ))
    ...
    def _build_model(self):
        if self.options.is_training:
            ...
            self.output_photo = decoder(features=self.input_photo_features,
                                        style=self.input_painting,
                                        options=self.options,
                                        reuse=False)

            # Get content features of output images. Need them to compute content feature loss.
            self.output_photo_content = encoder(image=self.output_photo,
                                                 options=self.options,
                                                 reuse=True)
            ...
            # Image loss.
            self.img_loss_photo = mse_criterion(transformer_block(self.output_photo), transformer_block(self.input_photo))
            self.img_loss = self.img_loss_photo

            # Style loss.
            '''
            self.style_loss_photo = style_statistic_loss(denormalize_arr_of_imgs(self.input_painting),
                                                         denormalize_arr_of_imgs(self.output_photo))
            '''
            self.style_loss_photo = style_statistic_loss(self.input_painting, self.output_photo)
            self.style_loss = self.style_loss_photo

            # Content features loss.
            self.content_feature_loss_photo = abs_criterion(self.output_photo_content, self.input_photo_features)
            self.content_feature_loss = self.content_feature_loss_photo

            # Adv content feature loss.
            fake_outputs = feature_discriminator(self.output_photo_content, self.options, reuse=False)
            real_outputs = feature_discriminator(self.input_photo_features, self.options, reuse=True)
            self.feature_d_loss, self.feature_g_loss = adv_feature_loss(real_outputs, fake_outputs)

            # TV loss
            self.tv_loss_photo = total_variation_loss(self.output_photo)
            self.tv_loss = self.tv_loss_photo
            ...
            self.feature_dis_vars = [var for var in t_vars if 'feature_dis' in var.name]
            ...
            with tf.control_dependencies(update_ops):
                ...
                self.g_optim_step = tf.train.AdamOptimizer(self.lr).minimize(
                    loss=self.options.discr_loss_weight * self.gener_loss +
                         self.options.content_feature_adv_loss_weight * self.feature_g_loss +
                         self.options.transformer_loss_weight * self.img_loss +
                         self.options.style_loss_weight * self.style_loss +
                         self.options.content_feature_loss_weight * self.content_feature_loss +
                         self.options.tv_loss_weight * self.tv_loss,
                    var_list=[self.encoder_vars + self.decoder_vars])

                self.feature_d_optim_step = tf.train.AdamOptimizer(self.lr).minimize(
                    loss=self.feature_d_loss,
                    var_list=[self.feature_dis_vars])
            ...
            # Style loss summary.
            s_s1 = tf.summary.scalar("style_loss/photo", self.style_loss_photo)
            s_s = tf.summary.scalar("style_loss/loss", self.style_loss)
            self.summary_style_loss = tf.summary.merge([s_s1 + s_s])

            # Content feature loss summary.
            s_cf1 = tf.summary.scalar("content_feature_loss/photo", self.content_feature_loss_photo)
            s_cf = tf.summary.scalar("content_feature_loss/loss", self.content_feature_loss)
            self.summary_content_feature_loss = tf.summary.merge([s_cf1 + s_cf])

            # TV loss summary.
            s_tv1 = tf.summary.scalar("tv_loss/photo", self.tv_loss_photo)
            s_tv = tf.summary.scalar("tv_loss/loss", self.tv_loss)
            self.summary_tv_loss = tf.summary.merge([s_tv1 + s_tv])
            ...
            with tf.name_scope('placeholder'):
                self.input_photo = tf.placeholder(dtype=tf.float32,
                                                  shape=[self.batch_size, None, None, 3],
                                                  name='photo')
                # Example guided for artistic style
                self.input_painting = tf.placeholder(dtype=tf.float32,
                                                     shape=[self.batch_size, None, None, 3],
                                                     name='painting')
    ...
    def train(self, args, ckpt_nmbr=None):
        ...
        for step in tqdm(range(self.initial_step, self.options.total_steps+1),
            ...
            if discr_success >= win_rate:
                ...
                _ = self.sess.run(self.feature_d_optim_step,
                                  feed_dict={
                                      self.input_painting: normalize_arr_of_imgs(batch_art['image']),
                                      self.input_photo: normalize_arr_of_imgs(batch_content['image']),
                                      self.lr: self.options.lr
            ...
            if step % 500 == 0:
                output_photos_ = self.sess.run(self.output_photo,
                    feed_dict={
                        self.input_painting: normalize_arr_of_imgs(batch_art['image']),
                        self.input_photo: normalize_arr_of_imgs(batch_content['image']),
                        self.lr: self.options.lr
    ...
    def inference(self, args, path_to_folder, to_save_dir=None, resize_to_original=True,
        ...
        for img_idx, img_path in enumerate(tqdm(names)):
            ...
            #img = scipy.misc.imresize(img, size=(self.image_size, self.image_size))
            img = np.expand_dims(img, axis=0)

            # Example guided for artistic style
            #artwork = scipy.misc.imread('images/reference/cezanne/3.jpg', mode='RGB')
            artwork = scipy.misc.imread(args.reference_image, mode='RGB')
            img_shape = artwork.shape[:2]
            alpha = float(self.image_size) / float(min(img_shape))
            artwork = scipy.misc.imresize(artwork, size=alpha)
            ...
            img = self.sess.run(
                ...
                feed_dict={
                    self.input_photo: normalize_arr_of_imgs(img),
                    self.input_painting: normalize_arr_of_imgs(artwork)
                })
